  ...
  return [
    {
      ...
      config: {
        ...
        inputSchema: {
          ...
          subject: z.string(),
          body: z.string().describe('Email body as a Handlebars template string. Supports {{contact.email}}/{{contact.<customField>}} merge tags, plus any key later passed as `data` when sending (available at the top level, e.g. sending data:{orderId:123} makes {{orderId}} available - not {{data.orderId}}). unsubscribeLink/pauseSubscriptionLink are NOT available on transactional emails. This tool cannot author bluefox.email\'s visual (Chamaileon) editor format or start from a saved template - content is always sent as plain html/text.'),
