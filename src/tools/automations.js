    ...
    },
    {
      ...
      config: {
        ...
        inputSchema: {
          ...
          value: z.any().optional().describe('set only. For is-opened/is-clicked/is-not-opened/is-not-clicked: the target email\'s emailId - the API rejects this if it does not match a send-email/notify node\'s emailId in this automation, so get it from a fresh manage_automation "get" rather than a stale/remembered id. For has-tag/does-not-have-tag: the tag name. Otherwise: the value to compare against "property".'),
    ...
    },
    {
      ...
      config: {
        ...
        inputSchema: {
          ...
          waitUntilDay: z.array(z.enum(['monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday', 'sunday'])).min(1).optional().describe('delay wait-until-day - one or more days; waits for whichever comes first (e.g. ["monday", "thursday"] on a Tuesday waits until Thursday). Today never counts, it rolls to next week.'),
          ...
          value: z.any().optional().describe('filter-audience / set-value / branch condition. For is-opened/is-clicked/is-not-opened/is-not-clicked: the target email\'s emailId (from an earlier send-email/notify node in this automation) - the API rejects this if it does not match a send-email/notify node\'s emailId in this automation, so get it from a fresh manage_automation "get" rather than a stale/remembered id. For has-tag/does-not-have-tag: the tag name. Otherwise: the value to compare against "property".'),
          ...
          subscriberListId: z.string().optional().describe('notify - required for this node to activate/merge successfully, even if only using "emails" for extra recipients (optional at add/update time, but the automation will fail validation without it once you try to go live).'),
          subscriberListName: z.string().optional().describe('Same as subscriberListId, by list name.'),
      ...
      handler: async (args) => {
        ...
        if (args.action === 'add' && args.nodeType === 'branch' && [args.property, args.operator, args.value, args.segmentId, args.segmentName, args.link, args.excludeUnengaged].some(v => v !== undefined)) {
          return textResult('property/operator/value/segmentId/link/excludeUnengaged have no effect when adding a branch - a fresh branch always starts with two default arms ("any") with no id to target yet. Add the branch with just nodeType "branch" (plus prevNodeId if needed), then configure each arm with a follow-up manage_automation_node update (condition: true, targeting one of the two returned arm ids) or add a third arm with nodeType "condition".')
        }
