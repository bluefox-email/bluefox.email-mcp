    ...
    },
    {
      ...
      config: {
        ...
        inputSchema: {
          ...
          value: z.any().optional().describe('set only. For is-opened/is-clicked/is-not-opened/is-not-clicked: the target email\'s emailId - only checked for a valid id shape, not that it actually belongs to this automation, so double-check it against a fresh manage_automation "get" rather than a stale/remembered id. For has-tag/does-not-have-tag: the tag name. Otherwise: the value to compare against "property".'),
    ...
    },
    {
      ...
      config: {
        ...
        inputSchema: {
          ...
          value: z.any().optional().describe('filter-audience / set-value / branch condition. For is-opened/is-clicked/is-not-opened/is-not-clicked: the target email\'s emailId (from an earlier send-email/notify node in this automation) - only checked for a valid id shape, not that it actually belongs to this automation, so double-check it against a fresh manage_automation "get" rather than a stale/remembered id. For has-tag/does-not-have-tag: the tag name. Otherwise: the value to compare against "property".'),
