      ...
      config: {
        ...
        inputSchema: {
          ...
          customSubscriptionPreferencesUrl: z.string().optional().describe(
            ...
            'Building this page? It never needs and should never hold the project\'s API key - the emailed link carries a `?token=...` query parameter scoped to that one subscriber\'s email, and that\'s the only credential the page ever sees. That token authorizes every PUBLIC list in the project (a private list needs the specific token minted for it). Send it as-is as a bearer token (`Authorization: Bearer <token>`) to the bluefox.email public API: ' +
            'GET /v1/projectId/{projectId}/subscriber-lists/public/{email} lists every list that email can manage with its current status (active/paused/unsubscribed) on each; PATCH /v1/projectId/{projectId}/subscriber-lists/{id}/subscribers/{email} changes status on any of those lists (pause needs a future pausedUntil); POST /v1/projectId/{projectId}/subscriber-lists/{id}/subscribers re-subscribes them to one they are not currently on. Same token for every call on that page load - do not decode, generate, or persist it.'
          ),
